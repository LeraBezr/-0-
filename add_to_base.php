...
// Получаем описание видео из формы 
$video_source = $_POST['video_source']; 
// Получаем источник видео из формы 
$video_link = ($_POST['video_link']) ? $_POST['video_link'] : null; 
// Получаем ссылку на видео из формы, если она была указана 
$video_path = null; 
// Переменная для хранения пути к видео
 ...
 if ($video_source == 'file') {     
    ...
    $target_dir = "uploads/"; 
    // Указываем путь для сохранения загруженных файлов     
    $target_file = $target_dir . basename($_FILES["video_file"]["name"]);
    // Создаем полный путь к загруженному файлу
    $uploadOk = 1;
     // Флаг, определяющий, прошла ли загрузка успешно     
        ...
        if (!empty($video_link)) {         
            ...
            if (strpos($video_link, 'youtube.com') !== false || strpos($video_link, 'youtu.be') !== false) { 
                ...
                elseif (strpos($video_link, 'rutube.ru') !== false) { 
                    $video_path = "https://rutube.ru/play/embed/" . basename(parse_url($video_link, PHP_URL_PATH));         }     } else {         echo "Ссылка на видео не указана.";     } }
                    if ($video_path !== null) { 
                        $sql = "INSERT INTO videos (user_id, username, video_name, video_description, video_source, video_path) VALUES ('$user_id', '$username', '$video_name', '$video_description', '$video_source', '$video_path')";     if ($conn->query($sql) === TRUE) {         header("Location: all_videos.php");         echo "Видео успешно добавлено в базу данных.";     } else {         echo "Ошибка: " . $sql . "<br>" . $conn->error;     } } 
                        $conn->close(); ?> 
