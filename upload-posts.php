...
$sql_posts = "SELECT posts.id, posts.title, posts.content, posts.image_url, COUNT(comments.id) AS comments_count
 FROM posts                LEFT JOIN comments ON posts.id = comments.post_id                WHERE posts.user_id = '$user_id'                GROUP BY posts.id                ORDER BY posts.created_at DESC"; 
...
if ($result_posts->num_rows > 0) {
    while($row = $result_posts->fetch_assoc()) {   
        ...
        echo "<h3><a href='http://localhost/Bezrodnova/final/posts/post.php?id=" . $row['id'] . "'>" . htmlspecialchars($row['title']) . "</a></h3>"; 
