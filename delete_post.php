...
if (isset($_GET['id'])) {     $post_id = $_GET['id'];
    ...
    if ($conn->query($delete_comments_sql) === TRUE) { 
        ...
        if ($conn->query($delete_post_sql) === TRUE) { 
            ...
            exit();         } else {             echo "Ошибка при удалении поста: " . $conn->error;         }     } else {         echo "Ошибка при удалении комментариев: " . $conn->error;     } } else {     echo "Ошибка: Не передан параметр id для удаления поста."; } ?> 
