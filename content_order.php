...
foreach($data as $elem){ 
    ...
    if ($elem['paid'] == 0){            
         ...
         src="http://localhost/Bezrodnova/Social/shop/img/krest.png" alt=""></td>';          
        }        
    else{             
        $result .='<td>Оплачен</td>';            
         $result .='<td><img style="width: 115px;" 
         src="http://localhost/Bezrodnova/Social/shop/img/galochka.png" alt=""></td>';
        } 
        $result .='</tr>';     
    }     
    $result .='</table>'; 
        ...
        foreach($data as $elem){         $result .='<tr>'; 
            ...
            $result .='<td>'.$elem['summa'].'</td>';         $result .='</tr>'; // 
            $sum += $elem['summa'];     } 
...
?>
<script src="js/jquery-3.7.1.min.js"></script> </body> </html> 
