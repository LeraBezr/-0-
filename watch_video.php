...
<head>     
    ...
    <!--  <link rel="stylesheet" href="../css/loader.css"> так как на странице видео может загружаться долго лоадер идет на усмотрение -->     
            ...
            // Получение информации о видео из базы данных по ID
                    ...
                    if ($result->num_rows > 0) {                 
                        ...
                        if ($video_source == 'file') {                     
                            ...
                            echo "<iframe src='$iframe_src' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share' title='YouTube video player' allowfullscreen class='video'></iframe>";                 }
                            echo "<p>$video_description</p>";             } else { 
                                echo "Видео не найдено.";             }         } else { 
                                    ...
                                    echo '<button type="submit" name="submit_comment">Отправить комментарий</button>'; echo '</form>'; 
                                    echo '</div>'; 
                                    if (isset($_POST['submit_comment'])){ 
                                        $author_name = isset($_SESSION['login']) ? 
                                        $conn->real_escape_string($_SESSION['login']) : 'Anonymous';    
