...
// Проверяем, был ли введен новый пароль, если да, то хешируем и помещаем в кусочек запроса,         // иначе в запросе новый пароль не указываем   
    if (!empty($pass_first)) { 
 ...
 // Проверяем, существует ли пользователь с таким email, исключая текущего пользователя 
    ...
    if ($check_email_result && $check_email_result->num_rows > 0) { 
        ...
        if ($conn->query($update_query) === TRUE) { 
            ...
            $_SESSION['login'] = $us_name;                 
            $_SESSION['age'] = $age; 
            header("Location: http://localhost/Bezrodnova/Social/profile/accaunt.php"); 
            exit(); 
        } else { 
